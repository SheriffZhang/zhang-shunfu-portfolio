...
const breakdowns:Record<string,{headline:string;mark:string;typeCopy:string;colorCopy:string;systemCopy:string;palette:string[];tokens:[string,string,string]}>={
  "ai-visual-lab":{headline:"材质、光线与空间",mark:"透 · 流",typeCopy:"以透明层叠和曲线运动建立轻盈、连续的空间秩序。",colorCopy:"银白与冰蓝模拟玻璃折射，深灰稳定画面重心。",systemCopy:"从静态主视觉延展到动态封面、空间影像与数字背景。",palette:["深空灰","雾银","云白","冰蓝"],tokens:["透明层叠","冰蓝折射","环形流线"]},
  "future-brand-system":{headline:"标识、留白与强调色",mark:"轻 · 序",typeCopy:"以几何标识和细线框架建立视觉秩序，避免画面过度装饰。",colorCopy:"以冷白和雾银作为画面主体，墨黑承载标题信息，钴蓝只保留在关键识别点。",systemCopy:"主视觉可延展到提案封面、品牌海报、社媒图和基础物料。",palette:["冷白","雾银","墨黑","钴蓝"],tokens:["几何符号","留白网格","钴蓝锚点"]},
  "digital-narrative-space":{headline:"跨屏、节奏与信息",mark:"屏 · 流",typeCopy:"以编号、图像裁切和模块化卡片组织跨屏阅读顺序。",colorCopy:"深海军蓝稳定信息底色，高饱和蓝建立数字感，珊瑚红只用于内容转场。",systemCopy:"同一内容模块可在桌面、移动端与社交媒体中重组。",palette:["深海军蓝","数字蓝","珊瑚红","暖象牙"],tokens:["图像裁切","蓝珊瑚对比","跨屏模块"]},
  "cultural-poster-series":{headline:"城市、时间与记忆",mark:"城 · 迹",typeCopy:"纵向结构连接海报，摄影碎片保留城市记忆的模糊感。",colorCopy:"米白与黑色形成历史质感，朱红作为时间标记贯穿整个系列。",systemCopy:"主视觉延展至海报、邀请函、导视与展览空间。",palette:["墨黑","旧纸米白","朱红","石灰灰"],tokens:["旧纸纹理","朱红竖线","城市剪影"]},
  "tea-packaging-system":{headline:"自然、材质与礼序",mark:"叶 · 礼",typeCopy:"茶芽被抽象为品牌符号，波纹表达山地与水汽。",colorCopy:"纤维米白体现自然触感，森林绿建立品类识别，哑金用于礼赠细节。",systemCopy:"盒装、罐装、内袋与礼袋形成完整产品家族。",palette:["纤维米白","森林绿","哑金","茶雾绿"],tokens:["叶形图标","纤维纸感","深绿哑金"]},
  "motion-identity":{headline:"形态、运动与时间",mark:"聚 · 释",typeCopy:"金属带通过旋转、聚合与释放形成循环运动语法。",colorCopy:"黑色空间突出轮廓，紫蓝边缘光记录运动轨迹，银灰保留金属反射。",systemCopy:"动态核心可拆分为片头、转场、循环背景和关键帧。",palette:["近黑","紫蓝","铬银","轨迹灰"],tokens:["金属带","紫蓝边缘光","粒子轨迹"]}
};
...
export default async function ProjectPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className={`detail detail-project-${project.number}`}><ProjectReader />
    ...
    <section className={`detail-breakdown breakdown-${project.number}`}><div className="breakdown-title"><span>视觉拆解</span><div><h2>{breakdown.headline}</h2><div className="breakdown-tokens" aria-label="项目元素语言">{breakdown.tokens.map((token) => <b key={token}>{token}</b>)}</div></div></div><div className="breakdown-board"><div className="breakdown-type"><small>视觉构成</small><div className="breakdown-signature" aria-hidden="true"><i/><i/><i/></div><strong>{breakdown.mark}</strong><p>{breakdown.typeCopy}</p></div><div className="breakdown-color"><small>色彩关系</small><div className="palette" aria-label={`项目色彩：${breakdown.palette.join("、")}`}>{breakdown.palette.map(color=><i key={color} aria-label={color}/>)}</div><div className="palette-labels" aria-hidden="true">{breakdown.palette.map(color=><span key={color}>{color}</span>)}</div><p>{breakdown.colorCopy}</p></div><div className="breakdown-system"><small>系统与延展</small>{extension.items.map((item,itemIndex)=><b key={item}>{String(itemIndex+1).padStart(2,"0")}<em>{item}</em></b>)}<p>{breakdown.systemCopy}</p></div></div></section>
